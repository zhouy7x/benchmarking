...
#WORKLOAD_DIR=$2  ### This is where Node-DC-SSR script is located
REL_DIR=$(dirname $0)
ROOT_DIR=`cd "${REL_DIR}/.."; pwd`
echo "ROOT_DIR=${ROOT_DIR}"
SCRIPT_DIR=${ROOT_DIR}/node-dc-ssr

RESOURCE_DIR=${SCRIPT_DIR}
WORKLOAD_DIR=${SCRIPT_DIR}/Node-DC-SSR

NODEDCSSR_DIR=${WORKLOAD_DIR}/Node-DC-SSR-electrode
...
function usage() {
  echo "USAGE:"
  echo "Currently this script will run Node-DC-SSR-electrode, with the affinities as follows:"
...
function check_node_app_status() {
  ...
  while true
  do
    x=`grep "Hapi.js server running at " $RESULTSLOG`
    if [ "x${x}" != "x" ]; then
      break
    fi
...
function clean_port() {
    PORT_PID=`lsof -i :$PORT | grep $PORT | awk '{print $2;}'`
    if [ -n "$PORT_PID" ]; then
        echo "Killing: $PORT_PID"
        kill $PORT_PID
        echo "Waiting 5s for port process to exit"
        sleep 5
    fi
}
...
function start_nodeapp_server() {
  ...
  echo -e " NODE_ENV=production $CPUAFFINITY $NODE_APP_CMD" 2>&1 | tee -a $LOGFILE
  echo -e "## BEGIN TEST ##\n" 2>&1 | tee -a $LOGFILE

  (
    pushd ${NODEDCSSR_DIR}
    echo `pwd`
    echo NODE_ENV=production $CPUAFFINITY $NODE_APP_CMD
    #$NODE_ROOT/deps/npm/bin/npm-cli.js install 2>&1 | tee -a $RESULTLOG
    #npm install 2>&1 | tee -a $RESULTSLOG
    echo "Now building node server!"
    $NODE_BUILD_CMD 2>&1 | tee -a $RESULTSLOG
    echo "Build node server succeed!"
    NODE_ENV=production $CPUAFFINITY $NODE_APP_CMD 2>&1 | tee -a $RESULTSLOG
...
function create_client_config() {
    echo -e "\n## CREATE DRIVER CONFIG FILE ##" 2>&1 | tee -a $RESULTSLOG
    rm -rf ${WORKLOAD_DIR}/Node-DC-EIS-client/config-local.json 2>&1 | tee -a $RESULTSLOG
    cp ${WORKLOAD_DIR}/Node-DC-EIS-client/config-ssr.json ${WORKLOAD_DIR}/Node-DC-EIS-client/config-local.json 2>&1 | tee -a $RESULTSLOG
    sed -i /server_ipaddress/s/.*/"    \"server_ipaddress\":\"$NODE_SERVER.sh.intel.com\","/g ${WORKLOAD_DIR}/Node-DC-EIS-client/config-local.json 2>&1 | tee -a $RESULTSLOG
}
...
optional DRIVERHOST
optional NODE_FILE node_modules/xclap/bin/clap.js
...
NODEDC_DRIVER_PATH=${WORKLOAD_DIR}/Node-DC-EIS-client
...
DRIVER_COMMAND="$RUNSPEC_AFFINITY python $DRIVERCMD -f ${WORKLOAD_DIR}/Node-DC-EIS-client/config-local.json -dir ${RESULTSDIR} ${DRIVERCMD_OPTIONS}"
...
# build command
NODE_BUILD_CMD="$NODE_AFFINITY ${NODE} ${NODE_FILE} build"
NODE_APP_CMD="$NODE_AFFINITY ${NODE} ${NODE_FILE} server"
