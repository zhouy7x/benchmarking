...
(function(exports) {
    ...
    (function() {
        ...
        function is_modified(compressor, tw, node, value, level, immutable) {
            ...
            if (lhs) return lhs;
            if (!immutable && parent instanceof AST_Call && parent.expression === node && !(value instanceof AST_Arrow) && !(value instanceof AST_Class) && !parent.is_expr_pure(compressor) && (!(value instanceof AST_Function) || !(parent instanceof AST_New) && value.contains_this())) {
                return true;
            }
            ...
            def(AST_SymbolRef, function(tw, descend, compressor) {
                ...
                if (d.fixed === undefined || !safe_to_read(tw, d)) {
                    ...
                    if (value instanceof AST_Lambda && recursive_ref(tw, d)) {
                        ...
                        d.single_use = value instanceof AST_Lambda && !value.pinned() || value instanceof AST_Class || d.scope === this.scope && value.is_constant_expression();
                    } else {
                        d.single_use = false;
                    }
        ...
        function tighten_body(statements, compressor) {
            ...
            function collapse(statements, compressor) {
                ...
                var scanner = new TreeTransformer(function(node, descend) {
                    ...
                    if (!stop_if_hit && (!lhs_local || !replace_all) && (parent instanceof AST_Binary && lazy_op(parent.operator) && parent.left !== node || parent instanceof AST_Conditional && parent.condition !== node || parent instanceof AST_If && parent.condition !== node)) {
                        stop_if_hit = parent;
                    }
        ...
        OPT(AST_Directive, function(self, compressor) {
            if (compressor.option("directives") && (!member(self.value, directives) || compressor.has_directive(self.value) !== self)) {
        ...
        OPT(AST_BlockStatement, function(self, compressor) {
            ...
            switch (self.body.length) {
              case 1:
                if (!compressor.has_directive("use strict") && compressor.parent() instanceof AST_If && can_be_extracted_from_if_block(self.body[0]) || can_be_evicted_from_block(self.body[0])) {
                    return self.body[0];
                }
                break;
        ...
        AST_Scope.DEFMETHOD("hoist_properties", function(compressor) {
            ...
            var tt = new TreeTransformer(function(node, descend) {
                ...
                if (node instanceof AST_VarDef) {
                    ...
                    if (sym.scope === self && (def = sym.definition()).escaped != 1 && !def.assignments && !def.direct_access && !def.single_use && !compressor.exposed(def) && !top_retain(def) && (value = sym.fixed_value()) === node.value && value instanceof AST_Object) {
                        descend(node, this);
        ...
        OPT(AST_Call, function(self, compressor) {
            ...
            function can_inject_args(block_scoped, safe_to_inject) {
                for (var i = 0, len = fn.argnames.length; i < len; i++) {
                    ...
                    if (!safe_to_inject || block_scoped[arg.name] || identifier_atom(arg.name) || scope.var_names()[arg.name]) {
                        return false;
                    }
            ...
            function can_inject_vars(block_scoped, safe_to_inject) {
                ...
                for (var i = 0; i < len; i++) {
                    ...
                    for (var j = stat.definitions.length; --j >= 0; ) {
                        ...
                        if (name instanceof AST_Destructuring || block_scoped[name.name] || identifier_atom(name.name) || scope.var_names()[name.name]) {
                            return false;
                        }
        ...
        OPT(AST_ConciseMethod, function(self, compressor) {
            ...
            if (compressor.option("arrows") && compressor.parent() instanceof AST_Object && !self.is_generator && !self.value.uses_arguments && !self.value.pinned() && self.value.body.length == 1 && self.value.body[0] instanceof AST_Return && self.value.body[0].value && !self.value.contains_this()) {
...
})(typeof Terser == "undefined" ? Terser = {} : Terser);
